        ...
        if df.height > 0:
            ...
            if "date" in df.columns:
                ...
                if not (20231101 <= date_min and date_max <= 20231130):
                    ...
                    quality_checks["data_errors"].append(f"Date range outside November 2023: {date_min}-{date_max}")

        return quality_checks

    except Exception as e:
        return {"total_rows": 0, "data_errors": [f"Failed to validate EOD option data quality: {str(e)}"]}
                ...
                for field in ["open", "high", "low", "close"]:
                    if field in schema_names:
                        ...
                        if len(non_zero_values) > 0:
                            print(
                                f"✓ {field} data looks valid (range: {non_zero_values.min():.2f} - {non_zero_values.max():.2f})"
                            )
                        else:
