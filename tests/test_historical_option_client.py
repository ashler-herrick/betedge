...
def test_light_load() -> bool:
    ...
    with HistoricalOptionClient() as client:
        try:
            start_date, end_date = TestConfig.get_test_date_range(
                days_back=3, days_span=TestConfig.LIGHT_DAYS
            )
...
def test_medium_load() -> bool:
    ...
    with HistoricalOptionClient() as client:
        try:
            start_date, end_date = TestConfig.get_test_date_range(
                days_back=3, days_span=TestConfig.MEDIUM_DAYS
            )
...
def test_heavy_load() -> bool:
    ...
    with HistoricalOptionClient() as client:
        try:
            start_date, end_date = TestConfig.get_test_date_range(
                days_back=3, days_span=TestConfig.HEAVY_DAYS
            )
