...
def generate_realistic_test_data(num_contracts: int) -> tuple[str, str]:
    ...
    for i in range(num_contracts):
        ...
        contract = {
            "ticks": ticks,
            "contract": {"root": symbol, "expiration": expiration, "strike": strike_minor, "right": right},
        }
...
def python_filter_to_ipc_bytes(
    option_json: str, stock_json: str, current_date_int: int, max_dte: int, base_pct: float
) -> bytes:
    ...
    current_date = date(int(current_date_str[:4]), int(current_date_str[4:6]), int(current_date_str[6:8]))
...
def run_benchmark_suite():
    ...
    for num_contracts, description in test_cases:
        ...
        results = {"contracts": num_contracts, "description": description, "json_size_mb": total_size_mb}
        ...
        py_ipc_result, py_ipc_time, py_ipc_memory = benchmark_implementation(
            "Python IPC", python_filter_to_ipc_bytes, option_data, stock_data, current_date, max_dte, base_pct
        )
    ...
    for r in all_results:
        ...
        row = f"{r['description']:<12} {ipc_read_ms:>8.1f}ms {pqt_read_ms:>11.1f}ms {faster:<15} {ratio:>8.1f}x"
        print(row)
    ...
    ipc_vs_pqt_python = (
        (largest_test["py_pqt_size_kb"] / largest_test["py_ipc_size_kb"] * 100)
        if largest_test["py_ipc_size_kb"] > 0
        else 0
    )
    ipc_vs_pqt_rust = (
        (largest_test["rust_pqt_size_kb"] / largest_test["rust_ipc_size_kb"] * 100)
        if largest_test["rust_ipc_size_kb"] > 0
        else 0
    )
    ...
    # Throughput
    rust_ipc_throughput = (
        largest_test["rust_ipc_ticks"] / largest_test["rust_ipc_time"] if largest_test["rust_ipc_time"] > 0 else None
    )
    rust_pqt_throughput = (
        largest_test["rust_pqt_ticks"] / largest_test["rust_pqt_time"] if largest_test["rust_pqt_time"] > 0 else None
    )
