...
Example Usage:
    ...
    stock_request = ExternalHistoricalStockRequest(
        root="AAPL", start_date="20240101", end_date="20240131", data_schema="ohlc"
    )

    # Request earnings data
    earnings_request = ExternalEarningsRequest(start_date="202401", end_date="202403")
    ```
"""
...
from betedge_data.client.utils import generate_month_list, generate_trading_date_list
...
class BaseClientRequest(BaseModel):
    ...
    force_refresh: bool = Field(
        False, description="If True, reprocess existing files (overwrite)"
    )
...
class ClientOptionRequest(BaseClientRequest):
    """
    ...
    Example Usage:
        ...
        eod_request = ExternalHistoricalOptionRequest(
            root="TSLA", start_date="20240101", end_date="20240331", data_schema="eod"
        )
        ```
    ...
    start_date: str = Field(
        ..., description="Start date in YYYYMMDD format", pattern=r"^\d{8}$"
    )
    end_date: str = Field(
        ..., description="End date in YYYYMMDD format", pattern=r"^\d{8}$"
    )
    ...
    interval: int = Field(
        900_000,
        description="Interval in milliseconds (default 900000 for 15 minutes)",
        ge=0,
    )
    return_format: str = Field(
        "parquet", description="Return format for the request, either parquet or ipc."
    )
    start_time: Optional[int] = Field(
        None, description="Start time in milliseconds since midnight ET", ge=0
    )
    end_time: Optional[int] = Field(
        None, description="End time in milliseconds since midnight ET", ge=0
    )

    # Pre-computed requests attribute
    requests: List[HistOptionBulkRequest] = Field(
...
class ClientStockRequest(BaseClientRequest):
    """
    ...
    Example Usage:
        ...
        eod_request = ExternalHistoricalStockRequest(
            root="SPY", start_date="20220101", end_date="20240131", data_schema="eod"
        )
        ```
    ...
    data_schema: str = Field(
        default="quote",
        description="Data data_schema type: options include 'quote', 'ohlc', 'eod'",
    )
    interval: int = Field(
        60_000,
        description="Interval in milliseconds (default 60000 for 1 minute)",
        ge=0,
    )
    return_format: str = Field(
        "parquet", description="Return format for the request, either parquet or ipc."
    ...
    # Pre-computed requests attribute
    requests: List[HistStockRequest] = Field(
        default_factory=list, init=False, repr=False
    )
...
class ClientEarningsRequest(BaseClientRequest):
    """
    ...
    start_date: str = Field(
        ..., description="Start date in YYYYMM format", pattern=r"^\d{6}$"
    )
    end_date: str = Field(
        ..., description="End date in YYYYMM format", pattern=r"^\d{6}$"
    )
    return_format: str = Field(
        "parquet", description="Return format (currently only parquet supported)"
    )

    # Pre-computed requests attribute
    requests: List[EarningsRequest] = Field(
        default_factory=list, init=False, repr=False
    )
    ...
    def _compute_requests(self) -> List[EarningsRequest]:
        ...
        return [
            EarningsRequest(
                year=date[0], month=date[1], return_format=self.return_format
            )
            for date in dates
        ]
