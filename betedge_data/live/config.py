...
class LiveClientConfig(BaseSettings):
    ...
    stock_interval: int = Field(
        default=60000,  # 1 minute,
        description="Default interval for ThetaData to return at, e.g. data returned as of every X",
    )
    option_interval: int = Field(
        default=60000,  # 15 minutes (maximum allowed by ThetaData),
        description="Default interval for ThetaData to return at, e.g. data returned as of every X",
    )
