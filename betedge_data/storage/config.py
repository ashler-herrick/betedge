...
class MinIOConfig(BaseModel):
    ...
    endpoint: str = Field(
        default_factory=lambda: os.getenv("MINIO_ENDPOINT", "localhost:9000"), description="MinIO server endpoint"
    )
    access_key: str = Field(
        default_factory=lambda: os.getenv("MINIO_ACCESS_KEY", "minioadmin"), description="MinIO access key"
    )
    secret_key: str = Field(
        default_factory=lambda: os.getenv("MINIO_SECRET_KEY", "minioadmin"), description="MinIO secret key"
    )
    bucket: str = Field(
    ...
    region: str = Field(default_factory=lambda: os.getenv("MINIO_REGION", "us-east-1"), description="MinIO region")
...
class MinIOPublishConfig(BaseModel):
    ...
    schema: str = Field(default="quote", description="Data schema type (e.g., 'quote', 'ohlc', 'trade')")
    root: str = Field(..., description="Option root symbol (e.g., 'AAPL')")
    date: str = Field(..., description="Date in YYYYMMDD format", pattern=r"^\d{8}$")
    interval: int = Field(..., description="Interval in milliseconds", ge=0)
    exp: str = Field(..., description="Expiration as string (e.g., '0' for all expirations)")
    filter_type: str = Field(default="filtered", description="Filter type applied to data")
