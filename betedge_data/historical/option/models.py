...
class HistoricalOptionRequest(BaseModel):
    ...
    def validate_cross_fields(self) -> 'HistoricalOptionRequest':
        ...
        if self.exp == 0:
            if self.start_date != self.end_date:
                raise ValueError(f"When exp=0 (all expirations), start_date must equal end_date. Got {self.start_date} != {self.end_date}")
        ...
        if self.start_date != self.end_date and self.exp != 0:
            ...
            try:
                ...
                if days_diff > 7:
                    raise ValueError("Date ranges over 7 days may cause performance issues. Consider splitting request into smaller ranges.")
            except ValueError as e:
