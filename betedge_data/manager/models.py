...
class HistoricalOptionRequest(BaseModel):
    ...
    exp: Union[str, List[str], int] = Field(
        0, 
        description="Single expiration (e.g., '20231117'), list of expirations, or 0 for all expirations"
    )
    ...
    base_pct: float = Field(
        0.1, 
        description="Base percentage for dynamic moneyness thresholds (e.g., 0.1 for 10%)",
        gt=0,
        le=1.0
    )
    interval: int = Field(
    ...
    max_workers: Optional[int] = Field(
        None,
        description="Maximum number of threads for multiple expirations",
        gt=0,
        le=20
    )
...
class DataProcessingResponse(BaseModel):
    ...
    data_type: Optional[Literal["stream", "parquet", "json"]] = Field(
        None,
