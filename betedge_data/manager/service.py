...
class DataProcessingService:
    ...
    def __init__(self, force_refresh: bool = False, minio_config: Optional[MinIOConfig] = None):
        """
    ...
    async def process_historical_option_request(
        self, request: HistoricalOptionRequest, request_id: UUID
    ) -> DataProcessingResponse:
        """
        ...
        try:
            ...
            with HistoricalOptionClient() as client:
                ...
                if len(dates_to_fetch) == 0:
                    logger.info(
                        f"All {len(dates)} days already exist for {request.root} (use force_refresh=True to reprocess)"
                    )
                ...
                logger.info(
                    f"Parallel fetch completed in {fetch_time:.2f}s - fetched {len(day_results)}/{len(dates_to_fetch)} days"
                )
            ...
            if days_processed == 0:
                logger.error(
                    f"Complete failure: no days processed for {request.root} across {len(dates)} dates ({dates})"
                )
                return DataProcessingResponse(
                    status="error",
                    request_id=request_id,
    ...
    def _filter_unpublished_dates(self, dates: list[str], request: HistoricalOptionRequest) -> list[str]:
        """
    ...
    def _fetch_single_day(
        self, date: str, request: HistoricalOptionRequest, client: HistoricalOptionClient
        ...
        try:
            ...
            logger.debug(
                f"Fetching data for {request.root} on {date} with params: exp={request.exp}, max_dte={request.max_dte}, base_pct={request.base_pct}"
            )
    ...
    async def _parallel_fetch_days(
        self, dates: list[str], request: HistoricalOptionRequest, client: HistoricalOptionClient
        ...
        with ThreadPoolExecutor(max_workers=min(30, len(dates))) as executor:
            ...
            future_to_date = {executor.submit(self._fetch_single_day, date, request, client): date for date in dates}
    ...
    async def _parallel_upload_days(
        self, day_results: Dict[str, Tuple[bytes, int]], request: HistoricalOptionRequest, request_id: UUID
    ) -> Dict[str, int]:
        """
        ...
        with ThreadPoolExecutor(max_workers=min(10, len(day_results))) as executor:
            ...
            for date, (parquet_bytes, data_size) in day_results.items():
                future = executor.submit(self._upload_single_day, parquet_bytes, date, request, request_id)
    ...
    def _upload_single_day(
        self, parquet_bytes: bytes, date: str, request: HistoricalOptionRequest, request_id: UUID
    ) -> int:
        """
    ...
    def create_error_response(
        self, request_id: UUID, error_code: str, message: str, details: Optional[str] = None
    ) -> ErrorResponse:
        """
        ...
        """
        return ErrorResponse(request_id=request_id, error_code=error_code, message=message, details=details)
