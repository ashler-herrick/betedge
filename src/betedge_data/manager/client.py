...
from betedge_data.manager.job_tracker import JobInfo
...
class BetEdgeClient:
    ...
    def request_data(self, request: ExternalBaseRequest) -> JobInfo:
        """
        ...
        job_info = asyncio.run(self.service.process_request(request, request_id))
        return job_info
    ...
    def get_job_status(self, job_info: JobInfo) -> JobInfo | None:
        """
        ...
        job = self.service.get_job_status(job_info=job_info)
        if not job:
            raise RuntimeError("Provided JobInfo not found.")
        
        return self.service.get_job_status(job_info=job_info)
